<template>
  <div class="user-role-add-dialog">
    <el-dialog title="新增菜单" :visible.sync="visible" width="800px" v-drag-dialog append-to-body>
          ...
          <el-col :span="12">
            <el-form-item prop="meta.icon" label="菜单图标">
              <el-input v-model="ruleForm.meta.icon" placeholder="请选择菜单图标" @focus="$refs.iconZoomRef.open()"
                :prefix-icon="ruleForm.meta.icon" @blur="onIconZoomBlur"></el-input>
      ...
      <span slot="footer">
        ...
        <el-button type="primary" size="small" @click="onSubmitForm('ruleForm')">新 增</el-button>
      </span>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  methods: {
    // 打开弹窗
    open(row) {
      this.superMenuData = this.filterMenu(row)
      this.visible = true
    },
    // 关闭弹窗
    close() {
